 ...
 * claim. The sibling is neither a `require` nor a `require-dev`, so their
 * `interface_exists` and `class_exists` locks never engage: the copies are what
 * the whole suite runs against **and** what PHPStan analyses through
 * `scanFiles`. A method added to `Metric` upstream would therefore leave every
 * test here green and fatal on the first install that has both addons — a green
 * suite over a contract that no longer exists is worse than no suite, because
 * it is believed.
 *
 ...
 *
 * The contract comparison runs through a second PHP process
...
class InsightsContractsMatchTest extends TestCase
{
    ...
    /**
     * The base class, byte for byte — and not merely signature for signature.
     *
     * A stricter standard than the one above, because it is a different kind of
     * thing. The contracts are interfaces: they carry no behaviour, so
     * everything a caller can be broken by *is* the signature, and comparing
     * shapes says all there is to say.
     *
     * `TableMetric` is code. The metrics in `src/Integrations/Insights` extend
     * it and inherit its logic, which means the copy is not a shape the suite
     * type-checks against — it is the code the suite actually executes. A copy
     * with identical signatures and a different `bucketExpression()` would run
     * every test in this package green while production bucketed a chart
     * differently, and nothing anywhere would say so. That failure is invisible
     * by construction: both sides compile, both sides answer, and only the
     * numbers differ.
     *
     * So the rule is that the file is the same file. `cp` is the whole of the
     * maintenance procedure, this test is what notices when somebody forgot,
     * and the copy is excluded from Pint in `pint.json` so that formatting
     * cannot quietly break the equality either.
     */
    #[Test]
    public function the_copied_base_class_is_the_real_file(): void
    {
        ...
        $this->assertSame(
            ...
            'tests/Fakes/insights-table-metric.php is no longer the file it copies. '
            .'Every metric in src/Integrations/Insights extends it and the suite executes this copy, '
            .'so a difference here means the tests are green over code that is not the code production runs. '
            .'Copy it across again: cp '.$echt.' '.$kopie,
        );
    }
    ...
    protected function keinGeschwister(): string
    {
        return 'goldnead/statamic-insights was not found — neither in vendor/ nor checked out beside this package. '
            ...
            .'tests/Fakes/ still matches the real contract. Run this where the sibling exists.';
    }
