 ...
 * `autoload-dev` would map `Goldnead\StatamicInsights\` to this directory for
 * the whole dev install, so the day the real package lands in `require-dev` —
 * for a cross-addon test, say — Composer would have two sources for one
 * namespace and quietly serve whichever it found first. A stand-in that shadows
 * the thing it stands in for is worse than no stand-in at all. Loaded by hand
 * it can only ever be in play where a test asked for it.
 *
 ...
 * it.
 *
 * The base class the metrics in `src/Integrations/Insights` actually extend
 * lives one file over, in `insights-table-metric.php`, and is held to a
 * stricter standard again: byte equality rather than signature equality.
 */
